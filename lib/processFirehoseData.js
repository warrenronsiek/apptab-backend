...
const processFirehoseData = function (event, context, callback) {
  ...
  const processNodeSelected = payload => {
    ...
    const visibleNodesTable = JSON.stringify(visibleNodes.map(node => ({
      ...node,
      eventId: payload.eventId,
      date: payload.date
    })));
    ...
    return [...visibleNodesTable.map(node => ({payload: node, name: 'nodes'})), {payload: sessionTable, name: 'sessions'}]
  };
